...
function appendToInput(number) {
    document.getElementById('vote-input').value += number;
}

// Função para limpar a input
function clearInput() {
    document.getElementById('vote-input').value = '';
}

async function voteClickNew(e) {
    e.preventDefault();
    const vote_input = document.getElementById("vote-input").value;

    let product = products.find((prodct) => vote_input == prodct.product_number)
